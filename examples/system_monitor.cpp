...
private:
    ...
    void update_cpu_metrics() {
        ...
        for (size_t i = 0; i < current_stats.size(); ++i) {
            total_diff += static_cast<double>(current_stats[i] - prev_cpu_stats[i]);
        }
    ...
    void update_memory_metrics() {
        ...
        if (total_mem > 0) {
            double usage = 100.0 * (1.0 - static_cast<double>(available_mem) / total_mem);
            mem_usage->set(usage);
        }
    }
    ...
    std::pair<long, long> read_net_stats() {
        ...
        while (std::getline(net, line)) {
            ...
            if (iface.find(":") != std::string::npos) {
                ...
                if (iface == "lo" || iface.find("docker") != std::string::npos) continue;
    ...
    long parse_mem_line(const std::string& line) {
        ...
        if (pos != std::string::npos) {
            ...
            if (endpos < value_str.size() && value_str.find("kB") != std::string::npos) {
                return value * 1024;
            }
            return value;
        }
...
int main() {
    ...
    monitor.stop();
    std::cout << "System monitoring completed. All the metrics saved to system_metrics.log\n";
    return 0;
}